...
public class RedisListSinkTest {
    ...
    private RedisListSink sink;

    @BeforeMethod
    public void setUp() {
        MockitoAnnotations.initMocks(this);
        sink = new RedisListSink(mockClient);
    ...
    public void testProcess() throws Exception {
        ...
        Mockito.when(mockClient.rpush(Mockito.anyString(), Mockito.anyString())).thenReturn(1L);
        ...
        Mockito.verify(mockClient).rpush("test.key", "test data");
    }
    ...
    public void testBackoffProcess() throws Exception {
        ...
        Mockito.verify(mockClient, Mockito.never()).rpush(Mockito.anyString(), Mockito.anyString());
    }

    @Test(expectedExceptions = EventDeliveryException.class)
    public void testFailProcess() throws Exception {
        Mockito.when(mockChannel.take()).thenReturn(mockEvent);
        Mockito.when(mockClient.rpush(Mockito.anyString(), Mockito.anyString())).thenReturn(0L);
        Mockito.when(mockEvent.getBody()).thenReturn("test data".getBytes());
